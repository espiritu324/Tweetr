...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    ...
    func postTweet(tweetText: String, success: @escaping () -> (), failure: @escaping (Error) -> ()) {
        let tweetRequestAPI = "https://api.twitter.com/1.1/statuses/update.json"
        TwitterAPICaller.client?.post(tweetRequestAPI, parameters: ["status":tweetText], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
        })
